...
public class B_TEL_OUT : MonoBehaviour
{
    //public GameObject player; // Joueur (ralentir la vélocité))
    public GameObject blocTelEnter; // Bloc d'entrée (relié)
    ...
    private void OnCollisionEnter2D(Collision2D collision) 
    {
        if (collision.transform.CompareTag("Player"))
        {
            ...
            blocTelEnter.GetComponent<BoxCollider2D>().isTrigger = true;
            // Place le Player au B_TEL_ENTER
            collision.transform.position = blocTelEnter.transform.position;
    ...
    {
        //Désactive la hitbox B_TEL_ENTER | Attend 1 Seconde | Active la Hitbox
        blocTelEnter.GetComponent<BoxCollider2D>().enabled = false;
        yield return new WaitForSeconds(1f);
        blocTelEnter.GetComponent<BoxCollider2D>().enabled = true;
    }


    // Fonction pour Reset le bloc Fall
    public void ResetBlocTelOut()
    { 
